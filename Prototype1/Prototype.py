"""
first official prototype of the menu editing
note: the tesselation had not been implemented yet
it contains all other editing prototypes combined
with a little changes and a new shape: diamond
and other features
"""
...
def SIN(x): return np.around(np.sin(math.radians(x)), decimals=2) # accurate sinus(x) function
def COS(x): return np.around(np.cos(math.radians(x)), decimals=2) # accurate cosinus(x) function
...
shape_orientation = 'still'   # the current shape orientation
...
dots_visible = True
dots_order = []               # the order of the dots around (0,0) offset, to make it easy for a tile to be drawn
                              #     by simply changing the its offset

canvas = Canvas(root, width=w, heigh=h, bg="white") # the main canvas
canvas.place(x=10, y=10)

tool_canvas = Canvas(root, width=1260, heigh=48, bg='#ffe6cc') # the convas for buttons
...
def _init_corners():
    ...
    if   main_shape == 'triangle':  angles_order = [150, 270, 30]
...
# moves the clicked dot to the mouse
...
def _add_circle():
    if dots_visible:
        dot_count = len(polygon_pieces[0])
        if dot_count < 7:
            corn = [] # indicates the corner where the editable edge starts
...
# changes the orientation of the shape                  note: the changes does not make much difference without the tesselation
def _change_orientation():
    global shape_orientation
    if main_shape != 'triangle':
        if   shape_orientation == 'still':   shape_orientation = 'rotated'
        elif shape_orientation == 'rotated': shape_orientation = 'still'
        or_label.config(text="orientation: {}".format(shape_orientation))
    else:
        or_label.config(text="orientation: triangles")
...
def _update_polygon():
    ...
    if main_shape == "triangle":
        ...
        # the other 2 edges
        # still orientation
        if shape_orientation == 'still':
            for j in [0,1]:
                dots.append(corners[j+2][0]); dots.append(corners[j+2][1]) #corner
                curr_edge_middle = [ int((corners[j][0]+corners[j+1][0])/2), int((corners[j][1]+corners[j+1][1])/2) ]
        ...
        # rotated orientation
        elif shape_orientation == 'rotated':
...
# changes the colour of a polygon                       note: while there is no tesselation, this applies only to the main polygon
...
def _set_main_shape(new_shape):
    ...
    if warning == "yes":
        ...
        if main_shape != 'triangle': or_label.config(text="orientation: {}".format(shape_orientation))
        else:                        or_label.config(text="orientation: triangles")
...
bucket_button.place(x=205,y=5,width=40,height=40)

img6 = PhotoImage(file="or.png")
or_button = Button(tool_canvas, bd=2, bg="white", width=3, height=1, image = img6, command = _change_orientation)
or_button.place(x=165,y=5,width=40,height=40)

pallete = Canvas(tool_canvas, bd=5, bg=selected_colour, width=25, height=25)
pallete.bind('<Button-1>', colour_picker)
pallete.place(x=245,y=5)


## shape buttons ##

img7 = PhotoImage(file="V_hexagon.png")
bucket_button = Button(tool_canvas, bd=2, bg="white", width=3, height=1, image = img7, command = lambda:_set_main_shape("V_hexagon"))
bucket_button.place(x=1220,y=5,width=40,height=40)

img8 = PhotoImage(file="H_hexagon.png")
bucket_button = Button(tool_canvas, bd=2, bg="white", width=3, height=1, image = img8, command = lambda:_set_main_shape("H_hexagon"))
bucket_button.place(x=1180,y=5,width=40,height=40)

img9 = PhotoImage(file="diamond.png")
bucket_button = Button(tool_canvas, bd=2, bg="white", width=3, height=1, image = img9, command = lambda:_set_main_shape("diamond"))
bucket_button.place(x=1140,y=5,width=40,height=40)

img10 = PhotoImage(file="square.png")
bucket_button = Button(tool_canvas, bd=2, bg="white", width=3, height=1, image = img10, command = lambda:_set_main_shape("square"))
bucket_button.place(x=1100,y=5,width=40,height=40)

img11 = PhotoImage(file="triangle.png")
bucket_button = Button(tool_canvas, bd=2, bg="white", width=3, height=1, image = img11, command = lambda:_set_main_shape("triangle"))
...
mouse_pos_label.place(x=500,y=720)

or_label = Label(root, text="orientation: {}".format(shape_orientation), relief=RIDGE, bd=0)
or_label.place(x=700,y=720)

shape_label = Label(root, text="selected shape: {}".format(main_shape), relief=RIDGE, bd=0)
shape_label.place(x=900,y=720)
...
zoom_scale = Scale(root, orient=HORIZONTAL, from_=40, to=170, length=200, command = _scale)
zoom_scale.set(zoom)
