"""
Official release of the Cheesellation
@author Mihai-Vlad Septelici
"""
...
brush_cp_id = "none"                    # the current colour pattern id that the mouse is on so the brush knows where to paint
...
paints_ids = []                         # stores the ids of the paints stains in step 2 to make them easy to erase
...
def _select_tool(new_tool):
    global curr_tool
    curr_tool = new_tool
    root.config(cursor=("@assets/{}.cur").format(new_tool))
...
def _make_Tessellation():
    ...
    tiles_proprieties.update( {Polygon : [offset[0],offset[1],0,0,0]} )
...
def colour_polygon(id, mouse_button):
    if curr_tool == "fill":
        ...
        #print("id",id,"has colour pattern id:",_get_colour_id(tiles_proprieties[id][2],tiles_proprieties[id][3],id,tiles_proprieties[id][4]))
...
def _save_as_PNG():
    ...
    try:
        ...
        messagebox.showinfo('Info', 'image is saved as ' + str(filename))   
    except:
        messagebox.showerror('Info', 'image was not saved')
    if dots_are_visible: _toggle_vis_circles()
    if shape_is_highlighted: _toggle_shape_highlight()
...
# draw/erase paints on canvas
def _update_canvas_paint(event):
    if curr_tool in ['pencil','paint_stick','eraser','stamp']:
        global brush_cp_id
        detections = canvas.find_overlapping(event.x-1, event.y-1, event.x+1, event.y+1)
        brush_sz = 0
    
        if curr_tool == "pencil": brush_sz = 1
        elif curr_tool in ["paint_stick","stamp"]: brush_sz = brush_size   
        
        # update current shape colour pattern id
        hoverd_tile_id = ''
        for id in detections:
            brush_cp_id = 'none'
            if id in tiles_ids or id == Polygon:
                hoverd_tile_id = id
                brush_cp_id = _get_colour_id(tiles_proprieties[id][2],tiles_proprieties[id][3],id,tiles_proprieties[id][4])
                break
        
        if (brush_cp_id != 'none'):
            mouse_difference = [event.x-tiles_proprieties[id][0],event.y-tiles_proprieties[id][1]]
            
            # draw on the tiles according to their colour pattern id
            for id in ([Polygon]+tiles_ids):
                if _get_colour_id(tiles_proprieties[id][2],tiles_proprieties[id][3],id,tiles_proprieties[id][4]) == brush_cp_id:
                    paint_pos = [tiles_proprieties[id][0]+mouse_difference[0], tiles_proprieties[id][1]+mouse_difference[1]]
                    tile_rot = tiles_proprieties[id][4] - tiles_proprieties[hoverd_tile_id][4]
                    paint_pos = _rotate(paint_pos[0],paint_pos[1],  tiles_proprieties[id][0],tiles_proprieties[id][1],  tile_rot)
                    
                    if (event.state == 0 and curr_tool == "stamp") or curr_tool in ["paint_stick","pencil"]:
                        _paint_here(paint_pos[0], paint_pos[1], brush_sz)
                    elif curr_tool == "eraser":
                        tiles_found = canvas.find_overlapping(paint_pos[0]-2, paint_pos[1]-2, paint_pos[0]+2, paint_pos[1]+2)
                        for i in tiles_found:
                            if i in paints_ids:
                                paints_ids.remove(i)
                                canvas.delete(i)
            
            # uncomment for debugging
            #print('bursh id = ',brush_cp_id)
            #print('id ',hoverd_tile_id,' has center at',tiles_proprieties[id][0],',',tiles_proprieties[id][1])
            #print('the differece between the mouse and its center is',mouse_difference[0],',',mouse_difference[1])
            #print('and has rotation',tiles_proprieties[id][4],'deg')
...
def move_to_step2():
    ...
    if answer == "yes":
        ...
        ### make tessellation visible, hide the dots and remove tile click tags binded
        global tiles_visible
...
bgimage1 = PhotoImage(file="assets/canvas_bg1.png")
tool_canvas.create_image(1, 1, image=bgimage1, anchor=NW)


######## tooltip displayer ##

tooltip_ballon = Balloon(root)
...
img1 = PhotoImage(file="assets/grab.png")
...
tooltip_ballon.bind_widget(move_button, balloonmsg="Drag tool\nUse it to move the dots and edit the main shape")

img2 = PhotoImage(file="assets/point_add.png")
...
tooltip_ballon.bind_widget(pencil_button, balloonmsg="Add dots\nAdds a dot on each editable edge\nAlways inserts them on the longest lines")

img3 = PhotoImage(file="assets/point_rem.png")
...
tooltip_ballon.bind_widget(erase_button, balloonmsg="Remove dots\nRemoves a dot from each editable edge")

img4 = PhotoImage(file="assets/point_rem_all.png")
erase_button_all = Button(tool_canvas, bd=2, bg="white", width=3, height=1, image = img4, command = _erase_all_circles)
erase_button_all.place(x=130,y=5,width=40,height=40)
tooltip_ballon.bind_widget(erase_button_all, balloonmsg="Remove all dots")

img5 = PhotoImage(file="assets/bucket.png")
...
tooltip_ballon.bind_widget(bucket_button, balloonmsg="Paint bucket\nUse it to give the shape and tiles a background colour\nNote: it depends on the colour pattern")


## shape buttons ##

img6 = PhotoImage(file="assets/V_hexagon.png")
Vh_button = Button(tool_canvas, bd=2, bg="white", width=3, height=1, image = img6, command = lambda:_set_main_shape("V_hexagon"))
Vh_button.place(x=1200,y=5,width=40,height=40)
tooltip_ballon.bind_widget(Vh_button, balloonmsg="Change the main shape to vertical hexagon")

img7 = PhotoImage(file="assets/H_hexagon.png")
Hh_button = Button(tool_canvas, bd=2, bg="white", width=3, height=1, image = img7, command = lambda:_set_main_shape("H_hexagon"))
Hh_button.place(x=1160,y=5,width=40,height=40)
tooltip_ballon.bind_widget(Hh_button, balloonmsg="Change the main shape to horizontal hexagon")

img8 = PhotoImage(file="assets/diamond.png")
D_button = Button(tool_canvas, bd=2, bg="white", width=3, height=1, image = img8, command = lambda:_set_main_shape("diamond"))
D_button.place(x=1120,y=5,width=40,height=40)
tooltip_ballon.bind_widget(D_button, balloonmsg="Change the main shape to diamond")

img9 = PhotoImage(file="assets/square.png")
S_button = Button(tool_canvas, bd=2, bg="white", width=3, height=1, image = img9, command = lambda:_set_main_shape("square"))
S_button.place(x=1080,y=5,width=40,height=40)
tooltip_ballon.bind_widget(S_button, balloonmsg="Change the main shape to square")

img10 = PhotoImage(file="assets/triangle.png")
T_button = Button(tool_canvas, bd=2, bg="white", width=3, height=1, image = img10, command = lambda:_set_main_shape("triangle"))
T_button.place(x=1040,y=5,width=40,height=40)
tooltip_ballon.bind_widget(T_button, balloonmsg="Change the main shape to triangle")
...
tooltip_ballon.bind_widget(toggle_tessellation, balloonmsg="Toggle the live preview of the tiles editing")
...
tess_combobox['values'] = ('still squares','rotated squares')
tooltip_ballon.bind_widget(tess_combobox, balloonmsg="Change tessellation mode\nWhich is different for each shape\nNote: each mode stores its own dots")
...
img11 = PhotoImage(file="assets/mouse_L.png")
pallete_L.create_image(2, 2, image=img11, anchor=NW)
tooltip_ballon.bind_widget(pallete_L, balloonmsg="Pick the colour for left mouse button")
...
img12 = PhotoImage(file="assets/mouse_R.png")
...
tooltip_ballon.bind_widget(outline_scale, balloonmsg="Change shape and tiles outline size\nNote: 0 means no outline")
...
tooltip_ballon.bind_widget(pattern_combobox, balloonmsg="Change colour pattern\nThe colours of the shapes will change depending on it\nAnd each pattern stores its own colours codes")
...
img13 = PhotoImage(file="assets/pencil.png")
...
tooltip_ballon.bind_widget(pen_button, balloonmsg="Pencil\nUnlike the stamp and the paint stick,\nIt only draws small lines")

img14 = PhotoImage(file="assets/paint_stick.png")
...
tooltip_ballon.bind_widget(paint_stick_button, balloonmsg="Paint stick\nDraws like a brush,\nAnd its size can be changed")

img15 = PhotoImage(file="assets/stamp.png")
...
tooltip_ballon.bind_widget(stamp_button, balloonmsg="Stamp\nSimilar to the paint stick\nBut it draws only on clicks")

img16 = PhotoImage(file="assets/eraser.png")
...
tooltip_ballon.bind_widget(brush_eraser_button, balloonmsg="Eraser\nUse it to erase paint")
...
for co in ["#FFFFFF","#000000","#808080","#C0C0C0","#800000","#FF0000","#808000","#FFFF00","#008000","#00FF00","#008080","#00FFFF","#000080","#0000FF","#800080","#FF00FF"]:
    ...
    colour_slot.create_line(2,2,2,36, fill="#000000")
    colour_slot.create_line(2,36,36,36, fill="#000000")
    colour_slot.create_line(36,36,36,2, fill="#000000")
    colour_slot.create_line(36,2,2,2, fill="#000000")
...
for i in range(8):
    ...
    colour_slot.create_line(36,36,36,2, fill="#000000")
    colour_slot.create_line(36,2,2,2, fill="#000000")
    tooltip_ballon.bind_widget(colour_slot, balloonmsg="Custom colour slot")
    colour_slots_ids.append(colour_slot)
...
tooltip_ballon.bind_widget(brush_size_scale, balloonmsg="Change the size of the brush\nFor the paint stick and the stamp")

brush_styles_label = Label(root, text="Brush styles:", relief=RIDGE, bd=0)
brush_styles_label.place(x=10000, y=10000)

img17 = PhotoImage(file="assets/circle_brush.png")
...
tooltip_ballon.bind_widget(brush_style_1, balloonmsg="Change brush style to circles")

img18 = PhotoImage(file="assets/square_brush.png")
...
tooltip_ballon.bind_widget(brush_style_2, balloonmsg="Change brush style to squares")
...
canvas.bind('<Button-1>', _update_canvas_paint)
canvas.bind('<B1-Motion>', _update_canvas_paint)
